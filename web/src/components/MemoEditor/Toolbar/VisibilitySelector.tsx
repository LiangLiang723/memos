  ...
  return (
    <DropdownMenu onOpenChange={props.onOpenChange}>
      <DropdownMenuTrigger asChild>
        <button className="inline-flex flex-shrink-0 whitespace-nowrap items-center px-1 text-sm text-muted-foreground opacity-80 hover:opacity-100 transition-colors">
          <VisibilityIcon visibility={value} className="opacity-60 mr-1.5" />
          <span className="whitespace-nowrap">{currentLabel}</span>
          <ChevronDownIcon className="flex-shrink-0 ml-0.5 w-4 h-4 opacity-60" />
        </button>
      </DropdownMenuTrigger>
