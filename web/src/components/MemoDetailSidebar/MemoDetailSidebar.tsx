    ...
    <aside className={cn("relative w-full h-auto max-h-screen overflow-auto flex flex-col gap-5", className)}>
      ...
      <div className="w-full space-y-1">
        ...
        <p className="text-sm text-foreground/70">
          {memo.createTime ? dayjs(timestampDate(memo.createTime)).format("YYYY-MM-DD HH:mm:ss") : "—"}
        </p>
      </div>
      ...
      {!isEqual(memo.createTime, memo.updateTime) && (
        <div className="w-full space-y-1">
          ...
          <p className="text-sm text-foreground/70">
            {memo.updateTime ? dayjs(timestampDate(memo.updateTime)).format("YYYY-MM-DD HH:mm:ss") : "—"}
          </p>
        </div>
      )}
